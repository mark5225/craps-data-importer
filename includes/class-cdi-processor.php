...
class CDI_Processor {
    ...
    public function process_import() {
        // Verify nonce
        if (!isset($_POST['nonce']) || !cdi_verify_nonce($_POST['nonce'])) {
        ...
        $settings = array(
            ...
            'similarity_threshold' => isset($_POST['similarity_threshold']) ? intval($_POST['similarity_threshold']) : cdi_get_option('similarity_threshold', 80),
        ...
        cdi_log('Starting import process with ' . count($csv_data['data']) . ' rows');
        ...
        foreach ($csv_data['data'] as $index => $row) {
            ...
            switch ($action) {
                ...
                default:
                    $results['errors']++;
                    cdi_log('Unknown action: ' . $action . ' for casino: ' . $casino_name, 'error');
                    continue 2;
            }
            ...
            if ($result['success']) {
                ...
                $results['errors']++;
                $results['details'][] = $result;
                cdi_log('Error processing casino: ' . $casino_name . ' - ' . $result['message'], 'error');
            }
        }
        ...
        $success_rate = cdi_calculate_success_rate($results['updated'] + $results['created'] + $results['errors'], $results['updated'] + $results['created']);
        
        cdi_log(sprintf('Import completed: %d updated, %d created, %d skipped, %d errors (%.1f%% success rate)', 
            $results['updated'], $results['created'], $results['skipped'], $results['errors'], $success_rate));
    ...
    private function add_to_review_queue($casino_name, $csv_row) {
        ...
        $result = $wpdb->insert(
            ...
            array(
                ...
                'csv_data' => cdi_json_encode_for_js($csv_row),
